...
class Container:
    ...
    def __repr__(self):
        return "Container: <%s>" % repr(self.__dict__)
...
def redir(to, *args, **kwargs):
    if not (to.startswith('/') or
        to.startswith("http://") or
        to.startswith("../") or to == '#'):
...
def add_csrf(request, **kwargs):
    """Add CSRF to dictionary and wrap in
    a RequestContext (needed for context processor!)."""
...
def getitem(iterable, index, default=None):
    ...
    return default if index out of range."""
    try:
